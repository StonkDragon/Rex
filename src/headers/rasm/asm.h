...
void asm_addInstruction(uint8_t inst, uint8_t reg1, uint8_t reg2, uint8_t reg3, uint32_t address) {
    asm_data[asm_ptr++] = inst;
    asm_data[asm_ptr++] = reg1;
    asm_data[asm_ptr++] = reg2;
    asm_data[asm_ptr++] = reg3;
...
uint32_t asm_writeData(string buffer, uint32_t size) {
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        if (strcmp(operand, "breakpoint") == 0) {
            asm_addInstruction(BREAKPOINT, 0, 0, 0, 0);
        } else if (strcmp(operand, "nop") == 0) {
            asm_addInstruction(NOP, 0, 0, 0, 0);
            ...
            asm_addInstruction(MOVE, regIdentifier(reg1), regIdentifier(reg2), 0, 0);
        } else if (strcmp(operand, "psi") == 0) {
            string imm = strtok(NULL, " ");
            bin_checkNull(imm, operand);
            bin_checkIsNumber(imm, operand);
            int num = atoi(imm);

            asm_addInstruction(STORE, 0, 0, 0, num);
        } else if (strcmp(operand, "psh") == 0) {
            string imm = strtok(NULL, " ");
            bin_checkNull(imm, operand);
            bin_checkIsRegister(imm, operand);

            asm_addInstruction(PUSH, regIdentifier(imm), 0, 0, 0);
            ...
            asm_addInstruction(STORE, regIdentifier(reg), 0, 0, num);
            ...
            string imm = strtok(NULL, " ");
            bin_checkNull(imm, operand);
            uint32_t num = bin_labelToInt(imm, operand);

            asm_addInstruction(LOAD_IMM, regIdentifier(reg), 0, 0, num);
            ...
            string address = strtok(NULL, " ");
            bin_checkNull(address, operand);
            uint32_t num = bin_labelToInt(address, operand);

            asm_addInstruction(LOAD, regIdentifier(reg), 0, 0, num);
        } else if (strcmp(operand, "pop") == 0) {
            string reg = strtok(NULL, " ");
            bin_checkNull(reg, operand);
            bin_checkIsRegister(reg, operand);

            asm_addInstruction(POP, regIdentifier(reg), 0, 0, 0);
        } else if (strcmp(operand, "dup") == 0) {
            asm_addInstruction(DUP, 0, 0, 0, 0);
        } else if (strcmp(operand, "swap") == 0) {
            asm_addInstruction(SWAP, 0, 0, 0, 0);
        } else if (strcmp(operand, "add") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(IADD, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "sub") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(ISUB, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "mul") == 0) {            
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(IMUL, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "div") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);
            
            asm_addInstruction(IDIV, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "mod") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(IREM, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "neg") == 0) {
            asm_addInstruction(INEG, 0, 0, 0, 0);
        } else if (strcmp(operand, "and") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(IAND, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "or") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(IOR, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "xor") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(IXOR, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "shl") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);

            asm_addInstruction(ISHL, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "shr") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(ISHR, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "not") == 0) {
            asm_addInstruction(INOT, 0, 0, 0, 0);
        } else if (strcmp(operand, "inc") == 0) {
            asm_addInstruction(IINC, 0, 0, 0, 0);
        } else if (strcmp(operand, "dec") == 0) {
            asm_addInstruction(IDEC, 0, 0, 0, 0);
        } else if (strcmp(operand, "cmp") == 0) {
            string reg1 = strtok(NULL, " ");
            bin_checkNull(reg1, operand);
            bin_checkIsRegister(reg1, operand);

            asm_addInstruction(CMP, regIdentifier(reg1), 0, 0, 0);
        } else if (strcmp(operand, "goto") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(GOTO, 0, 0, 0, add);
        } else if (strcmp(operand, "jeq") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_EQ, 0, 0, 0, add);
        } else if (strcmp(operand, "jne") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_NE, 0, 0, 0, add);
        } else if (strcmp(operand, "jlt") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_LT, 0, 0, 0, add);
        } else if (strcmp(operand, "jgt") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_GT, 0, 0, 0, add);
        } else if (strcmp(operand, "jle") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_LE, 0, 0, 0, add);
        } else if (strcmp(operand, "jge") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_GE, 0, 0, 0, add);
        } else if (strcmp(operand, "syscall") == 0) {
            asm_addInstruction(SYSTEM, 0, 0, 0, 0);
        } else if (strcmp(operand, "jsr") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_TRUE, 0, 0, 0, add);
        } else if (strcmp(operand, "rts") == 0) {    
            asm_addInstruction(RETURN, 0, 0, 0, 0);
        } else if (strcmp(operand, "jz") == 0) {
            string label = strtok(NULL, " ");
            bin_checkNull(label, operand);
            uint32_t add = bin_labelToInt(label, operand);

            asm_addInstruction(IF_NULL, 0, 0, 0, add);
    ...
    while ((asm_ptr + HEADER_SIZE) % byteAlignment != 0) {
        asm_data[asm_ptr + HEADER_SIZE] = 0;
        asm_ptr++;
    }
