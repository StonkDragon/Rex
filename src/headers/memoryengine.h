...
void        me_writeInt(uint64_t addr, uint32_t value);
...
uint32_t    me_readInt(uint64_t addr);
...
void me_heapDump(const string dumpFile) {
    ...
    if (sp > 0) {
        for (size_t i = 0; i < sp; i++) {
            fprintf(f, "  0x%04zx: 0x%16llx", i, stack[i]);
            fprintf(f, "\n");
        }
    } else {
        fprintf(f, "  <empty>\n");
...
void me_writeFloat(uint64_t addr, float value) {
    me_writeInt(addr, *(uint64_t*) &value);
}
